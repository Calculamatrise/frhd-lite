...
export default class {
    ...
    draw() {
        ...
        ctx.lineCap = "round",
        ctx.strokeStyle = window.lite.storage.get("theme") === "midnight" ? "#666" : lite.storage.get("dark") ? "#666" : this.settings.sceneryLineColor,
        this.drawLines(this.sceneryLines, this.scene.camera.zoom, ctx),
        ctx.stroke(),
        ctx.beginPath(),
        ctx.strokeStyle = window.lite.storage.get("theme") === "midnight" ? "#ccc" : lite.storage.get("dark") ? "#fbfbfb" : this.settings.physicsLineColor,
